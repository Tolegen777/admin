...
const ComplaintUsersInfo = () => {
    ...
    return (
        <>
            {isWindowOpen &&
                <CustomWindow closeWindow={() => setWindowOpen(false)} isWindowOpen={isWindowOpen} isError={isError}
                              isSuccess={isSuccess} isLoading={isLoading}
                              label={"Вы действительно хотите заблокировать пользователя?"} blockUser={blockUser}
                              blockUserId={userData?.culpritId} changeComplaint={changeComplaint}
                              complaintId={userData.complaintId}
                />}
            {isWindowOpen2 &&
                <CustomWindow closeWindow={() => setWindowOpen2(false)} isWindowOpen={isWindowOpen2} isError={isError2}
                              isSuccess={isSuccess2} isLoading={isLoading2}
                              label={"Вы действительно хотите отклонить жалобу?"} changeComplaint={changeComplaint}
                              complaintStatus={complaintStatus.CANCELED}
                              complaintId={userData.complaintId}
                />}
                    ...
                    <Grid container xs={6}>
                        ...
                        <Grid sx={{margin: '5px 20px'}}>
                            ...
                            <Typography sx={{
                                ...
                                color: "primary.main",
                                fontWeight: '800'
                            }}>{`${userData?.secondName} ${userData?.firstName} ${userData?.middleName}`}</Typography>
                            {(!userData?.isBlocked && userData.complaintStatus !== complaintStatus.CANCELED) &&
                            ...
                            {(userData.complaintStatus === complaintStatus.COMPLETED || userData?.isBlocked) &&
                                <Typography sx={{marginTop: "20px", color: "green"}}>Пользователь
                                    заблокирован!</Typography>
                            }
